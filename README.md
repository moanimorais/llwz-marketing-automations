# llwz-marketing-automations

Documentation for marketing automations, CRM workflows, and email sequences — Live Life With Zest.

---

## About

Live Life With Zest is a coaching brand founded by Madelaine Romero, based in New York.
This repository documents all marketing automations, CRM workflows, email sequences, and integration setups.

**Tools used:**
- **Eventbrite** — event registration
- **Zapier** — integration layer
- **GoHighLevel / LeadConnector** — CRM, workflows, email delivery

---

## Workflows

| Document | Description | Status |
|----------|-------------|--------|
| [Webinar Reminder Workflow](workflows/webinar-reminder-workflow.md) | Automated 1-day and 1-hour email reminders for webinar attendees | ✅ Active |

---

*Last updated: April 2026*
*Maintained by: Marketing Team — Live Life With Zest*
